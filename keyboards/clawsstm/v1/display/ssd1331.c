// адаптировано под чибиос это https://github.com/anothermist/DISPLAYS
#include "ssd1331.h"
...
// настройка портов управления oled1331
void init_ports(void) {
  palSetLineMode(OLED_ON, PAL_MODE_OUTPUT_PUSHPULL);
  palSetLine(OLED_ON); // 1 = 14V on

  palSetLineMode(OLED_N_RESET, PAL_MODE_OUTPUT_PUSHPULL);
  palClearLine(OLED_N_RESET);

  palSetLineMode(OLED_N_CS, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
  palClearLine(OLED_N_CS);

  palSetLineMode(OLED_DC, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
  palSetLine(OLED_DC);

  // SPID2
  palSetLineMode(OLED_CLK, PAL_MODE_ALTERNATE(5) | PAL_STM32_OSPEED_HIGHEST);   // SCK
  palSetLineMode(OLED_DIN, PAL_MODE_ALTERNATE(5) | PAL_STM32_OSPEED_HIGHEST);   // MOSI
}
...
void SSD1331_Init(void)
{
	...
	chThdSleepMilliseconds(1);//HAL_Delay(1);
}
